...
func (dp *DatabaseProvider) Get_count() (kol int) {
	row := dp.db.QueryRow(`select count(*) from hello;`)
	if err := row.Scan(&kol); err != nil {
		log.Fatal(err.Error())
	}
	return
}
...
func main() {
	...
	h := Handlers{dp: dp}
	http.HandleFunc("/post", h.handler)
